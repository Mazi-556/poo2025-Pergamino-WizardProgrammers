...
import ar.edu.unnoba.poo2025.torneos.domain.ports.out.competitionRepositoryPortPort;
...
import ar.edu.unnoba.poo2025.torneos.xdto.RegistrationResponseDTO;  //TODO cambiar los dtos a arq hexagonal


@Service
public class RegistrationServiceIml implements RegistrationService {
    private final CompetitionService competitionService;
    private final RegistrationRepositoryPort registrationRepositoryPort;
    private final TournamentRepositoryPort tournamentRepositoryPort;
    private final CompetitionRepositoryPort competitionRepositoryPort;


    public RegistrationServiceIml(CompetitionService competitionService, registrationRepositoryPort registrationRepositoryPort, tournamentRepositoryPort tournamentRepositoryPort, competitionRepositoryPort competitionRepositoryPort) {
    ...
    public List<AdminCompetitionRegistrationDTO> getCompetitionRegistrations(Long tournamentId, Integer competitionId) throws Exception {
        ...
        Tournament t = tournamentRepositoryPort.findById(tournamentId)  
                .orElseThrow(() -> new Exception("Torneo no encontrado"));
        
        Competition c = competitionRepositoryPort.findById(competitionId)
        ...
        List<Registration> registrations = registrationRepositoryPort.findByCompetitionIdWithParticipant(c.getIdCompetition());
    ...
    public RegistrationResponseDTO registerParticipant(Long tournamentId, Integer competitionId, Participant participant) throws Exception {
        ...
        long currentRegistrations = registrationRepositoryPort.countByCompetitionId(competitionId);
        if (currentRegistrations >= c.getQuota()) {
            throw new Exception("No hay cupo disponible en esta competencia.");
        }

        if (registrationRepositoryPort.existsByParticipantAndCompetition(participant.getIdParticipant(), competitionId)) {
            throw new Exception("Ya estás inscripto en esta competencia.");
        }

        double price = c.getBasePrice();
        long otherRegistrationsInTournament = registrationRepositoryPort.countByParticipantAndTournament(participant.getIdParticipant(), tournamentId);
        ...
        Registration saved = registrationRepositoryPort.save(registration);
    ...
    public List<ParticipantRegistrationDTO> getParticipantRegistrations(Integer participantId) {
        List<Registration> list = registrationRepositoryPort.findByParticipantId(participantId);
...
public ParticipantRegistrationDetailDTO getRegistrationDetail(Integer registrationId, Integer participantId) throws Exception {
    ...
    Registration r = registrationRepositoryPort.findById(registrationId)
